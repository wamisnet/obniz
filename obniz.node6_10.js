...
class OMRON_2JCIE {
  ...
  findWait() {
    ...
    return _asyncToGenerator(function* () {
      ...
      _this.periperal = yield _this.obniz.ble.scan.startOneWait(target);

      return _this.periperal;
    })();
  }
  ...
  connectWait() {
    ...
    return _asyncToGenerator(function* () {
      if (!_this2.periperal) {
        yield _this2.findWait();
      }
      if (!_this2.periperal) {
        throw new Error('2JCIE not found');
      }
      if (!_this2.periperal.connected) {
        yield _this2.periperal.connectWait();
      }
    })();
  }
  ...
  disconnectWait() {
    ...
    return _asyncToGenerator(function* () {
      if (_this3.periperal && _this3.periperal.connected) {
        _this3.periperal.disconnectWait();
      }
    })();
  }
  ...
  getLatestData() {
    ...
    return _asyncToGenerator(function* () {
      yield _this4.connectWait();

      let c = _this4.periperal.getService(_this4.omron_uuid('3000')).getCharacteristic(_this4.omron_uuid('3001'));
      let data = yield c.readWait();
      let json = {
        row_number: data[0],
        temperature: _this4.signedNumberFromBinary(data.slice(1, 3)) * 0.01,
        relative_humidity: _this4.signedNumberFromBinary(data.slice(3, 5)) * 0.01,
        light: _this4.signedNumberFromBinary(data.slice(5, 7)) * 1,
        uv_index: _this4.signedNumberFromBinary(data.slice(7, 9)) * 0.01,
        barometric_pressure: _this4.signedNumberFromBinary(data.slice(9, 11)) * 0.1,
        soud_noise: _this4.signedNumberFromBinary(data.slice(11, 13)) * 0.01,
        discomfort_index: _this4.signedNumberFromBinary(data.slice(13, 15)) * 0.01,
        heatstroke_risk_factor: _this4.signedNumberFromBinary(data.slice(15, 17)) * 0.01,
        battery_voltage: _this4.unsignedNumberFromBinary(data.slice(17, 19)) * 0.001
      };
